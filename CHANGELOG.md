...
- **OCR decode path unified through the shared Go image loader.** All OCR entry points now hand Tesseract in-memory **lossless PNG** bytes (`SetImageFromBytes`) instead of a file path. The full-image and detect-regions paths decode through the same loader as every other tool, and the region path (`image_ocr_region`) now OCRs its crop straight from memory via the new `ExtractTextFromImage` primitive — eliminating the previous temp-file round-trip (encode → disk write → disk read → decode → re-encode) for pixels already held in memory. Leptonica therefore only ever decodes PNG, which is what makes the libpng-only static build safe; OCR also now reasons about the exact pixels the measurement/color tools use, and gains every input format the loader supports. PNG re-encode is lossless — no downsampling, no lossy round-trip.
